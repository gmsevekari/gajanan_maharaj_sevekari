...
- **Firestore Stream Flickering:** Never initialize a Firestore `Stream` directly inside the `build()` method of a widget (e.g., `stream: _firestore.collection(...).snapshots()`). This causes the stream to tear down and rebuild on every route change or hot reload, resulting in violent UI flickering and redundant database reads. Always use a `StatefulWidget` and cache the stream in `initState()`.
