...
class _CrossPlatformYoutubePlayerState
  ...
  void initState() {
    ...
    if (kIsWeb) {
      ...
      _mobileController = mobile_player.YoutubePlayerController(
        ...
        flags: mobile_player.YoutubePlayerFlags(autoPlay: widget.autoPlay),
      );
      _mobileController!.addListener(() {
        if (_mobileController!.value.playerState ==
            mobile_player.PlayerState.ended) {
          widget.onEnded?.call();
        }
      });
    }
  ...
  Widget build(BuildContext context) {
    return kIsWeb
        ? web_player.YoutubePlayer(controller: _webController!)
