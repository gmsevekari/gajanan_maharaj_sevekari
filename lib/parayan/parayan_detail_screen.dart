...
class _ParayanDetailScreenState extends State<ParayanDetailScreen>
  ...
  void _checkRegistration(String deviceId) {
    _parayanService
        .getParticipantsByDevice(widget.event.id, deviceId)
        .first
        .then((list) {
      if (mounted) {
  ...
  String _getSmartDate(String locale) {
    if (widget.event.type == ParayanType.oneDay ||
      ...
      return _formatDate(widget.event.startDate, locale);
    } else {
      final start = locale == 'mr'
          ? DateFormat('d MMMM', 'mr').format(widget.event.startDate)
          : DateFormat('MMMM d').format(widget.event.startDate);
      final end = _formatDate(widget.event.endDate, locale);
      return "$start - $end";
    }
  }

  String _getDescriptiveStatus(AppLocalizations localizations, String locale) {
    final date = _formatDate(widget.event.startDate, locale);

    switch (widget.event.status) {
      case 'upcoming':
        return widget.event.type == ParayanType.oneDay ||
                widget.event.type == ParayanType.guruPushya
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          locale == 'mr' ? widget.event.titleMr : widget.event.titleEn,
        ...
        children: [
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(height: 2),
                                      Text(
                                        widget.event.type == ParayanType.oneDay
                            ...
                            if (!isLandscape &&
                                (widget.event.descriptionEn.isNotEmpty ||
                                    widget.event.descriptionMr.isNotEmpty)) ...[
                            ...
                            _buildInfoChip(
                              context,
                              Icons.info_outline,
                              _getDescriptiveStatus(localizations, locale),
          ...
          StreamBuilder<List<ParayanMember>>(
            ...
            builder: (context, snapshot) {
              ...
              final canJoin = widget.event.status == 'enrolling' ||
                  widget.event.status == 'allocated';
